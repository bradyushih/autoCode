...
$(function() {
 ...
 let letters = "23456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
 let verifyLength = "4";
 let verifyCode = "";
 for(let i=0; i<verifyLength; i++) {
			verifyCode += letters.charAt((Math.random()*letters.length));
		}
	
 let options = {
    canvasId: "auth-code",/**canvas的id*/
    txt: verifyCode,/**傳入驗證碼內容*/
    height: 50,/**驗證碼高度 */
    width: 250,/**驗證碼寬度 */
...
$text.addEventListener('keyup', function () {
    if (helper.validate($text.value)) {
        document.getElementById('verify_code').innerHTML = "驗證碼正確"+"<input class='verify_code' hidden value='checked'></input>";
        ...
        document.getElementById('verify_code').innerHTML = "驗證碼錯誤"+"<input class='verify_code' hidden value='rejected'></input>";
